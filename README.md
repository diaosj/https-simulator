...
**HTTPS Visual Simulator** 是一个**纯前端交互式动画沙盒**，采用**赛博朋克 / 黑客终端**的暗黑视觉风格。它的使命是将晦涩难懂的 HTTPS 底层密码学原理——**CIA 三要素（机密性、完整性、身份验证）**、**TLS 握手协议**、**RSA/AES 混合加密体系**——通过**极其直观的逐帧动画分镜**，展现给非技术人员和初级开发者。
...
| 技术 | 版本 | 选型理由 |
|:---|:---:|:---|
| **React** | 18.2 | 声明式 UI + 严谨的状态机驱动，完美契合多步骤动画场景的流程编排。 |
| **TypeScript** | 5.2 | 为复杂的加密流程状态提供编译期类型安全保障，杜绝运行时类型错误。 |
| **Framer Motion** | 10.16 | 打造极具物理感的数据包飞行动画、关键帧插值与流畅的状态切换过渡。 |
| **TailwindCSS** | 3.3 | 原子化 CSS 构建硬核赛博朋克/极客终端暗黑 UI，`bg-slate-950` + `border-cyan-500` + `font-mono text-green-400`。 |
| **CryptoJS** | 4.2 | 在浏览器端执行**真实的** AES 加密、SHA-256 哈希运算，而非伪随机模拟。 |
| **Lucide React** | 0.292 | 提供简洁一致的 SVG 图标体系，替代传统 Emoji，契合极客终端视觉风格。 |
| **Vite** | 5.0 | 基于 ESM 的极速开发服务器与构建工具，毫秒级热更新。 |
...
在真实世界中，TLS 握手在**毫秒级**完成，AES 加密在**微秒级**完成——人类根本无法感知这些过程。本项目的设计哲学是：

**利用 React 单线程状态机的天然优势，将瞬间完成的网络传输和加密过程"定格"并"拉长"为数秒甚至数十秒的动画分镜。**

这是一个**纯前端项目**，没有后端、没有真实的网络请求。React 的状态机就是整个模拟宇宙的"时间之神"——它精确控制每一帧动画的触发时机、每一个数据包的飞行轨迹、每一次加密/解密的视觉呈现。

这种架构赋予了项目两个核心优势：

- **零部署成本**：无需服务器，`npm run dev` 即刻启动，任何人都能在 30 秒内体验。
- **极致教学体验**：每一个密码学步骤都被拆解为可暂停、可观察的动画帧，让抽象概念变得触手可及。
