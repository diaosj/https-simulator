...
**HTTPS Visual Simulator** 是一个**纯前端交互式动画沙盒**，采用**赛博朋克 / 黑客终端**的暗黑视觉风格。它的使命是将晦涩难懂的 HTTPS 底层密码学原理——**机密性、完整性、身份验证**、**TLS 握手协议**、**RSA/AES 混合加密体系**——通过**极其直观的逐帧动画分镜**，展现出来。

无需后端，无需部署，打开浏览器即可体验一场关于**信息安全攻防**的视觉盛宴。你将亲眼看到中间人（MITM）如何发起攻击，又是如何被现代密码学机制逐一击溃。

<p align="center">
  <!-- 👇 请在此处放置项目演示动图（GIF / 视频截图），添加素材后将下方示例替换为实际路径 -->
  <!-- <img src="path/to/demo.gif" alt="HTTPS Visual Simulator 演示动图" width="800" /> -->
</p>
...
1. **Server** 在本地安全保管自己的 🔴 私钥（Private Key），并准备好对应的 🟡 公钥（通常以数字证书形式展示，动画中会同时可视化两把钥匙）。
2. 仅 🟡 公钥（或包含公钥的证书）通过网络传输至 **Client**，途经中间人的监听节点；🔴 私钥始终留在服务器本地，从不在网络上传输。
...
1. Client 发送 AES 加密后的密文与 **SHA-256 哈希值**用于完整性校验。
2. 中间人截获密文后，**盲目翻转部分比特位** —— 密文被"污染"。
3. 篡改后的密文到达 Server，Server 解密后得到乱码。
4. Server 重新计算哈希并与原始哈希值比对 —— **不匹配！**
5. Server 立即 **丢弃该数据包**，弹出红色告警：
   ```
   ⚠️ 完整性校验失败！数据已被篡改，连接已终止。
   ```

> **说明：** 本演示使用简化的哈希校验来演示完整性保护原理。在真实 TLS 中，完整性保护由 **HMAC** 或 **AEAD 认证标签**（如 AES-GCM）实现，它们使用共享密钥来防止攻击者在篡改数据后重新计算标签。
